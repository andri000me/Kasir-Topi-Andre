...
class M_penjualan extends CI_Model{
	...
	function simpan_penjualan($nofak,$total,$jml_uang,$kembalian){
		...
		$this->db->query("INSERT INTO tbl_jual (jual_nofak,jual_total,jual_jml_uang,jual_kembalian,user_id,jual_keterangan) VALUES ('$nofak','$total','$jml_uang','$kembalian','$idadmin','eceran')");
	...
	function simpan_penjualan_grosir($nofak,$total,$jml_uang,$kembalian){
		$idadmin=$this->session->userdata('idadmin');
		$this->db->query("INSERT INTO tbl_jual (jual_nofak,jual_total,jual_jml_uang,jual_kembalian,user_id,jual_keterangan) VALUES ('$nofak','$total','$jml_uang','$kembalian','$idadmin','lunas')");
		foreach ($this->cart->contents() as $item) {
			$data=array(
				'jual_nofak' 			=>	$nofak,
				'barang_id'		=>	$item['id'],
				'barang_nama'	=>	$item['name'],
				'barang_harjul_grosir'	=>	$item['amount'],
				'd_jual_qty'			=>	$item['qty'],
				'd_jual_diskon'			=>	$item['disc'],
				'd_jual_total'			=>	$item['subtotal']
			);
	...
	function cetak_faktur(){
		...
		$hsl=$this->db->query("SELECT (j.jual_nofak) AS jual_nofak,DATE_FORMAT(jual_tanggal,'%d/%m/%Y %H:%i:%s') AS jual_tanggal,jual_total,jual_jml_uang,jual_kembalian,jual_keterangan,barang_nama,barang_harjul_grosir,d_jual_qty,d_jual_diskon,d_jual_total FROM tbl_jual j, tbl_detail_jual dj WHERE j.jual_nofak=dj.jual_nofak AND j.jual_nofak='$nofak'");
		return $hsl;
	}
