...
<tr>
    <td colspan="2" style="width:800px;paddin-left:20px;"><center><h4>LAPORAN PENJUALAN PERTAHUN <?php echo $b['tahun'];?></h4></center><br/></td>
</tr>
<tr>
    <td colspan="2" style="width:800px;paddin-left:20px;"><center><h4></h4></center><br/></td>
</tr>
...
<?php 
    ...
    if($group=='-' || $group!=$d['bulan']){
        $bulan=$d['bulan'];
        $query=$this->db->query("SELECT (j.jual_nofak) AS jual_nofak,DATE_FORMAT(jual_tanggal,'%M %Y') AS bulan,DATE_FORMAT(jual_tanggal,'%d %M %Y') AS jual_tanggal,barang_id,barang_nama,barang_harjul_grosir,d_jual_qty,d_jual_diskon,SUM(d_jual_total) AS total FROM tbl_jual j, tbl_detail_jual dj WHERE j.jual_nofak=dj.jual_nofak AND DATE_FORMAT(jual_tanggal,'%M %Y')='$bulan'");
        $t=$query->row_array();
        $tots=$t['total'];
        if($group!='-')
        echo "</table><br>";
        echo "<table align='center' width='900px;' border='1'>";
        echo "<tr><td colspan='7'><b>Bulan: $bulan</b></td> <td style='text-align:right;'><b>Total:</b></td><td style='text-align:right;'><b>".'Rp '.number_format($tots)."</b></td></tr>";
...
$nomor=1;
    ...
    $group=$d['bulan'];
        ...
        <tr>
                ...
                <td style="vertical-align:top;padding-left:5px;text-align:right;"><?php echo 'Rp '.number_format($d['barang_harjul_grosir']); ?></td>
